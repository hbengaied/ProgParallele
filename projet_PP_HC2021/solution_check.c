...
int solution_check_kernel(solution_t* const s, problem_t* const p) {
  /* OK: errors = 0. */
  int errors = 0;
...
void simulation_init_kernel(const problem_t* const p) {
  memset(car_state, 0, NB_CARS_MAX * sizeof(car_state_t));
  memset(street_state, 0, NB_STREETS_MAX * sizeof(street_state_t));
...
void simulation_dequeue_kernel(const problem_t* const p) {
  #pragma omp parallel for
  for (int street = 0; street < p->S; street++) {
    // If there is a street to dequeue
    if (street_state[street].out == 1) {
...
int simulation_run_kernel(const solution_t* const s, const problem_t* const p) {
  int score = 0;
...
int solution_score_kernel(solution_t* s, const problem_t* const p) {
  int score = 0;

  score = simulation_run_kernel(s, p);
